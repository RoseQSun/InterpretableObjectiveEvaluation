from music21 import stream, meter, key, interval, repeat
...
def time_sig(score):
    sig_list = []
    try:
        for i in score.recurse().getElementsByClass(meter.TimeSignature):
...
def num_note_per_bar(score):
    note_count = 0
    bars = s.getElementsByClass(stream.Part)[0].getElementsByClass(stream.Measure)
    for m in bars:
        note_count += len(m.getElementsByClass(note.Note))
    return note_count / len(bars)